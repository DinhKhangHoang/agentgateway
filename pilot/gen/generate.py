...
def emit_insecure_tls(base, mode, indent):
    """`policies.tls.insecureSkipVerify` lines for a MEASURED private-CA host.

    Returns [] when the flag is off or the host is not in `PRIVATE_CA_HOSTS`, so
    the knob is scoped to the hosts whose handshake was observed to fail and
    every other upstream keeps verifying. A blanket disable would also cover
    api.openai.com, which verifies fine today and would then stop telling us if
    that changed.
    """
    if not mode or not base:
        return []
    parts = split_base_url(base)
    if not parts or parts[1] not in PRIVATE_CA_HOSTS:
        return []
    p = " " * indent
    return [
        p + "# Host is in PRIVATE_CA_HOSTS: its chain does not verify against",
        p + "# the system trust store. Verification disabled by",
        p + "# --insecure-upstream-tls, which is what Kong's ai-proxy does today.",
        p + "tls:",
        p + "  insecureSkipVerify: " + mode,
    ]
...
def classify(rows, unreachable, ai_proxy, rep):
    ...
    for row in sorted(rows, key=lambda r: (r.route, r.plugin)):
        ...
        if row.cfg.get("failover_on"):
            # Kong `failover_on` HTTP statuses are now emitted, not lost: they
            # become both the health `unhealthyCondition` (knob 1, on each
            # concrete target) and the `traffic.retry.codes` (knob 2, Gateway-
            # wide AgentgatewayPolicy). Recorded as a caveat rather than a gap.
            rep.loss("`failover_on` statuses mapped to health+retry (two-knob)",
                     tag + " -- Kong retries on statuses "
                     + ",".join(str(x) for x in row.cfg["failover_on"])
                     + "; agentgateway now emits these as both the target health "
                       "`unhealthyCondition` and the retry `codes`. Caveat: the "
                       "default RETRY_CODES ([429,500,502,503,504]) is used when the "
                       "Kong list is broader; verify the mapping covers every status "
                       "the farm actually returns.")
        keep.append(row)
...
def emit_models(models, creds, ns, gw_name, gw_ns, rep, insecure_tls=None):
    out = ["# GENERATED by pilot/gen/generate.py -- do not edit by hand.",
           ...
           "#",
           "# FAILOVER: a model whose Kong config declares a `fallbacks` chain is",
           "# emitted as concrete Internal target CRs (one per target, each with",
           "# `policies.health` + `policies.routes` + its auth/transform) plus ONE",
           "# Public virtual CR with `spec.virtualModel.failover.targets[]`,",
           "# primary at priority 0 and each fallback at the next priority. This is",
           "# `virtualModel.failover` (priority-ordered), NOT `weighted` -- a Kong",
           "# fallback chain states order, not weight, and `targets[].priority`",
           "# takes exactly that. The two-knob form (health + retry) is completed",
           "# by `emit_failover_policies`, which attaches a `traffic.retry`",
           "# AgentgatewayPolicy to the Gateway."]
    # Listener-wide union of every non-built-in path any model opens.
    opened = sorted({p for m in models for p in m["paths"]})
    for m in models:
        fbs, fb_row = model_fallback_chain(m, rep)
        if fbs:
            out.extend(emit_failover_model(m, fbs, fb_row, creds, ns, gw_name, gw_ns,
                                           opened, rep, insecure_tls))
        else:
            out.extend(emit_flat_model(m, creds, ns, gw_name, gw_ns, opened,
                                       insecure_tls))
    return "\n".join(out) + "\n"


def _emit_document_separator(out):
    # Every YAML document begins with `---`, including the first (it follows the
    # file header comment block). A leading `---` is valid YAML and keeps the
    # splitter simple. Collapse a duplicated separator if the previous line was
    # already one (e.g. when a sub-emitter's first action is to separate).
    if not out or out[-1] != "---":
        out.append("---")


def _policy_lines(m, creds, ns, opened, insecure_tls, with_health=False,
                  consumer_suffix=None):
    """Build the `policies:` block lines (without the `  policies:` header) for a
    concrete model. `with_health` adds the health eviction policy that turns the
    passive per-response scorer into the first knob of two-knob failover."""
    indent = "    "
    policy = []
    consumer = "model/" + m["name"] + (("/" + consumer_suffix) if consumer_suffix else "")
    if m["auth"] is not None:
        entry = creds.ref(m["auth"].auth, consumer)
        if entry:
            policy.extend(emit_auth(entry, 4))
    policy.extend(emit_insecure_tls(m["base"], insecure_tls, 4))
    if m["upstream_name"] != m["match"]:
        policy.append(indent + "# Kong sent `%s` upstream while the client addresses"
                      % m["upstream_name"])
        policy.append(indent + "# `%s`. CEL string literal, hence the inner quotes."
                      % m["match"])
        policy.append(indent + "transformations:")
        policy.append(indent + "  - field: model")
        policy.append(indent + "    expression: " + yq("'%s'" % m["upstream_name"]))
    if with_health:
        # First knob of two-knob failover. `failover_on` HTTP statuses become the
        # unhealthy expression; a single consecutive failure evicts (Kong benches
        # on the first failing response). Eviction duration mirrors Kong's
        # `failover_cooldown` when set, else the agentgateway default.
        cooldown = m["rows"][0].cfg.get("failover_cooldown")
        cd = ("%ds" % cooldown) if isinstance(cooldown, (int, float)) else "60s"
        codes = " || ".join("response.code == %d" % c
                            for c in (m["rows"][0].cfg.get("failover_on") or RETRY_CODES))
        policy.append(indent + "# First knob of two-knob failover: mark the target")
        policy.append(indent + "# unhealthy on the Kong `failover_on` statuses so the")
        policy.append(indent + "# next priority group is tried. Eviction on the first")
        policy.append(indent + "# consecutive failure mirrors Kong's bench-on-failure.")
        policy.append(indent + "health:")
        policy.append(indent + "  unhealthyCondition: " + yq(codes))
        policy.append(indent + "  eviction:")
        policy.append(indent + "    duration: " + cd)
        policy.append(indent + "    consecutiveFailures: 1")
    if opened:
        policy.append(indent + "# REPLACES the built-in route table rather than extending")
        policy.append(indent + "# it, so every default this model still wants is restated.")
        policy.append(indent + "routes:")
        for path, rt in DEFAULT_ROUTE_TABLE:
            policy.append(indent + "  %s: %s" % (yq(path), rt))
        for path in opened:
            policy.append(indent + "  %s: %s" % (yq(path), NON_BUILTIN_ROUTE_TYPE[path]))
    return policy


def emit_flat_model(m, creds, ns, gw_name, gw_ns, opened, insecure_tls):
    """One Public concrete AgentgatewayModel with a flat `spec.provider`."""
    out = []
    _emit_document_separator(out)
    ...
    policy = _policy_lines(m, creds, ns, opened, insecure_tls)
    if policy:
        out.append("  policies:")
        out.extend(policy)
    return out
...
def emit_failover_model(m, fbs, fb_row, creds, ns, gw_name, gw_ns, opened, rep,
                        insecure_tls):
    """Concrete Internal target CRs + one Public virtualModel.failover CR.

    Reuses `emit_backends`'s fallback decoding (provider/name/upstream_url/auth
    per `fallback_identity`). The primary target carries the model's own
    provider/base/auth; each fallback carries its own. Primary is priority 0;
    each fallback increments -- the CRD says 'lower values preferred', so the
    live inversion (fallback=p0/primary=p1) is NOT reproduced.
    """
    out = []
    base_match = m["match"]
    targets = []  # (target_model_name, priority) for the virtual CR

    # --- primary concrete target (priority 0) -------------------------------
    primary_name = m["name"] + "-primary"
    targets.append((primary_name, 0))
    pm = dict(m)
    pm["name"] = primary_name
    pm["match"] = base_match + "-primary"
    out.extend(_emit_concrete_target(pm, creds, ns, gw_name, gw_ns, opened,
                                     insecure_tls, internal=True, with_health=True))

    # --- fallback concrete targets (priority 1, 2, ...) ---------------------
    for i, fb in enumerate(fbs):
        if not isinstance(fb, dict):
            continue
        fm = fb.get("model") or {}
        fprov = fm.get("provider")
        if fprov not in MODEL_PROVIDER:
            rep.unsupported.append(
                (fb_row.route, fb_row.plugin,
                 "canonical fallback #%d uses Kong provider `%s`, which has no "
                 "agentgateway model provider; the fallback target is dropped"
                 % (i + 1, fprov)))
            continue
        ...
        fr.model_literal = fb_row.model_literal
        fr.auth = fb.get("auth") or {}

        fb_name = m["name"] + ("-fallback-%d" % (i + 1) if len(fbs) > 1 else "-fallback")
        targets.append((fb_name, i + 1))
        fmodel = {
            "name": fb_name, "match": ("%s-fallback-%d" % (base_match, i + 1))
                    if len(fbs) > 1 else (base_match + "-fallback"),
            "vendor": m["vendor"], "model": m["model"],
            "provider": MODEL_PROVIDER.get(fprov, "OpenAI"),
            "base": fbase, "upstream_name": fr.kong_model_name or fb_row.model_literal,
            "auth": _Auth(fr.auth), "paths": [], "rows": [fb_row],
        }
        out.extend(_emit_concrete_target(fmodel, creds, ns, gw_name, gw_ns, opened,
                                         insecure_tls, internal=True, with_health=True))

    # --- public virtual failover model --------------------------------------
    _emit_document_separator(out)
    out.append("apiVersion: agentgateway.dev/v1alpha1")
    out.append("kind: AgentgatewayModel")
    out.append("metadata:")
    ...
    if gw_ns and gw_ns != ns:
        out.append("      namespace: " + yq(gw_ns))
    out.append("  match:")
    out.append("    model: " + yq(base_match))
    if m["paths"]:
        out.append("    paths:")
        for p in m["paths"]:
            out.append("      - " + yq(p))
    out.append("  visibility: Public")
    out.append("  virtualModel:")
    out.append("    failover:")
    out.append("      targets:")
    for tname, prio in targets:
        out.append("        - modelRef:")
        out.append("            name: " + yq(tname))
        out.append("          priority: %d" % prio)
    return out


def _emit_concrete_target(m, creds, ns, gw_name, gw_ns, opened, insecure_tls,
                           internal=False, with_health=False):
    """One concrete AgentgatewayModel (a failover target). Mirrors emit_flat_model
    but with visibility Internal and an optional health policy."""
    out = []
    _emit_document_separator(out)
    out.append("apiVersion: agentgateway.dev/v1alpha1")
    out.append("kind: AgentgatewayModel")
    ...
    if m["base"]:
        out.append("  baseURL: " + yq(m["base"]))
    out.append("  visibility: " + ("Internal" if internal else "Public"))
    policy = _policy_lines(m, creds, ns, opened, insecure_tls, with_health=with_health,
                           consumer_suffix=m["name"].rsplit("-", 1)[-1]
                           if "-" in m["name"] else None)
    if policy:
        out.append("  policies:")
        out.extend(policy)
    return out
...
class _Auth(object):
    """Adapts a raw Kong `auth` dict to the `.auth` attribute shape `creds.ref`
    expects (a Row exposes `.auth` as a dict; `_policy_lines` reads `m["auth"].auth`)."""
    def __init__(self, auth):
        self.auth = auth


def emit_failover_policies(models, ns, gw_name, rep):
    """AgentgatewayPolicy CRs with `traffic.retry` -- the second knob of two-knob
    failover. One policy per Gateway, targeting the Gateway, retrying the Kong
    `failover_on` statuses so a failed priority group is retried on the next.

    A single Gateway-wide policy covers every failover model: retry is a route
    trait, not a model trait, and `targetRefs` on the Gateway attaches it to all
    the Gateway's routes. Models without a fallback chain are unaffected --
    retry only fires on the RETRY_CODES statuses, which a healthy single-target
    model does not return."""
    out = []
    seen_gw = set()
    for m in models:
        fbs, _ = model_fallback_chain(m, rep)
        if not fbs:
            continue
        gw_key = (ns, gw_name)
        if gw_key in seen_gw:
            continue
        seen_gw.add(gw_key)
        _emit_document_separator(out)
        out.append("apiVersion: agentgateway.dev/v1alpha1")
        out.append("kind: AgentgatewayPolicy")
        out.append("metadata:")
        out.append("  name: " + yq("%s-failover-retry" % gw_name))
        out.append("  namespace: " + yq(ns))
        out.append("spec:")
        out.append("  targetRefs:")
        out.append("    - group: gateway.networking.k8s.io")
        out.append("      kind: Gateway")
        out.append("      name: " + yq(gw_name))
        out.append("  traffic:")
        out.append("    retry:")
        out.append("      attempts: %d" % RETRY_ATTEMPTS)
        out.append("      backoff: " + yq(RETRY_BACKOFF))
        out.append("      codes:")
        for c in RETRY_CODES:
            out.append("        - %d" % c)
        out.append("      maxReplayBytes: " + yq(RETRY_MAX_REPLAY))
    return "\n".join(out) + "\n" if out else ""


def emit_provider_block(row, base, canonical, kong_provider, name, creds, indent,
                        consumer, insecure_tls=None):
    p = " " * indent
    out = [p + "- name: " + yq(name)]
    ...
    if parts:
        ...
        if scheme == "https":
            ...
            insecure = emit_insecure_tls(base, insecure_tls, indent + 4)
            policy.extend(insecure or [p + "    tls: {}"])
...
def fallback_identity(fbs):
    """The part of a Kong fallback chain an emitted group can actually carry.

    `emit_backends` reads exactly four things per fallback: the provider, the
    model name, `model.options.upstream_url`, and `auth`. Every other key is a
    Kong-side translation knob with no agentgateway equivalent -- notably
    `responses_upstream_format`, which the responses plugin sets and the chat
    plugin does not. Comparing whole dicts called those two chains different
    and emitted neither; comparing what is consumed calls them the same, and
    the dropped knobs are already on record under unsupported features.
    """
    ident = []
    for fb in fbs:
        if not isinstance(fb, dict):
            ident.append(None)
            continue
        fm = fb.get("model") if isinstance(fb.get("model"), dict) else {}
        opts = fm.get("options") if isinstance(fm.get("options"), dict) else {}
        ident.append({"provider": fm.get("provider"),
                      "name": fm.get("name"),
                      "upstream_url": opts.get("upstream_url"),
                      "auth": fb.get("auth") or {}})
    return json.dumps(ident, sort_keys=True)


def pick_fallback_chain(b, rep):
    """The one Kong fallback chain a backend's rows agree on -> (chain, row).

    A backend groups every route_type that shares an upstream, and Kong states
    `fallbacks` per PLUGIN, so the chain can be declared on a row that is not
    `rows[0]`. Reading only `rows[0]` dropped it silently: on
    `legacy-z-ai-glm-5-2` the messages plugin declares none while the chat and
    responses plugins both name ModelArts, so the emitted backend had one group
    and no failover -- while report.md went on claiming the fallback "survives
    on the legacy AgentgatewayBackend as priority groups".

    Agreement is judged on `fallback_identity`, not the raw dicts. Chains that
    disagree there are NOT merged: `spec.ai.groups` is one ordered list per
    backend, so two rows naming different targets cannot both be honoured;
    that is recorded and nothing is emitted, rather than picking one.
    """
    ...
    for r in b.rows:
        ...
        if not isinstance(fbs, list) or not fbs:
            continue
        seen.setdefault(fallback_identity(fbs), []).append(r)
    if not seen:
        return [], None
    if len(seen) > 1:
        rows = [r for group in seen.values() for r in group]
        rep.loss("Rows sharing a backend declare DIFFERENT Kong fallback chains",
                 "`%s` -- %d distinct chains across %s. `spec.ai.groups` is one "
                 "ordered list per backend, so no chain is emitted and the "
                 "legacy surface for this model has NO failover. Split the "
                 "backend per route_type, or reconcile the Kong plugins."
                 % (b.name, len(seen),
                    ", ".join("`%s`" % r.plugin for r in rows)))
        return [], None
    rows = list(seen.values())[0]
    return rows[0].cfg["fallbacks"], rows[0]


def model_fallback_chain(model, rep):
    """The one Kong fallback chain a canonical model's rows agree on -> chain.

    Mirrors `pick_fallback_chain` for the AgentgatewayModel surface. A `model`
    dict (built in `group`) carries its rows under `rows`; Kong declares
    `fallbacks` per plugin, so the chain can live on a row that is not
    `rows[0]`. Agreement is judged on `fallback_identity` (the fields an emitted
    target can actually carry) -- chains disagreeing there are recorded and None
    ...
    if len(seen) > 1:
        rows = [r for group in seen.values() for r in group]
        rep.loss("Rows sharing a canonical model declare DIFFERENT Kong fallback chains",
                 "`%s` -- %d distinct chains across %s. A virtualModel.failover "
                 "targets list is one ordered set per model, so no chain is emitted "
                 "and the canonical surface for this model has NO failover. Split "
                 "the model per route_type, or reconcile the Kong plugins."
                 % (model["match"], len(seen),
                    ", ".join("`%s`" % r.plugin for r in rows)))
        return None, None
    rows = list(seen.values())[0]
    return rows[0].cfg["fallbacks"], rows[0]
...
# Kong `failover_on` -> agentgateway retry.codes. Kong retries the next target on
# these HTTP statuses; agentgateway retries the next priority group on the same.
# 400/404 are deliberately excluded -- a 4xx from the model is a client error
# (bad request shape, unknown model), not a target failure, and retrying it on a
# fallback would launder a client error into a different upstream's problem.
RETRY_CODES = [429, 500, 502, 503, 504]
RETRY_ATTEMPTS = 3           # 1 initial + 2 retries; covers a 3-tier chain.
RETRY_BACKOFF = "50ms"
RETRY_MAX_REPLAY = "100Mi"   # CRD cap; lets large prompts stay retriable.


def emit_backends(backends, creds, ns, rep, insecure_tls=None):
    out = ["# GENERATED by pilot/gen/generate.py -- do not edit by hand.",
           "#",
    ...
    for b in backends:
        ...
        out.extend(emit_provider_block(row, b.base, b.canonical_base, b.kong_provider,
                                       ...
                                       insecure_tls))
        fb_chain, fb_row = pick_fallback_chain(b, rep)
...
def emit_report(rep, ns, insecure_tls=None):
    ...
    if rep.private_ca_hosts and insecure_tls:
        L.append("- **Upstream TLS verification is DISABLED on %d host%s**, by"
                 % (len(rep.private_ca_hosts),
                    "" if len(rep.private_ca_hosts) == 1 else "s"))
        L.append("  `--insecure-upstream-tls %s`. Every model and every backend"
                 % insecure_tls)
        L.append("  provider dialling one of the hosts below carries")
        L.append("  `policies.tls.insecureSkipVerify: %s`; every other upstream still"
                 % insecure_tls)
        L.append("  verifies. The self-hosted `*.nip.io` MaaS endpoints present a")
        L.append("  private-CA chain and failed at connect with `invalid peer")
        L.append("  certificate: UnknownIssuer` or `CaUsedAsEndEntity` (measured")
        L.append("  in-cluster 2026-08-26); agentgateway verifies by default and Kong's")
        L.append("  ai-proxy does not verify at all, so this setting is")
        L.append("  bug-compatible with what production does today and adds no")
        L.append("  exposure the current path does not already have. It is still a")
        L.append("  posture regression against agentgateway's default, and it is")
        L.append("  **not** the answer for prod: that is")
        L.append("  `policies.tls.caCertificateRefs` naming a ConfigMap with the")
        L.append("  private CA.")
        if insecure_tls == "Hostname":
            L.append("  **`Hostname` does not address either measured failure.** It")
            L.append("  trusts the CA chain and ignores only hostname/SAN mismatch;")
            L.append("  `UnknownIssuer` is an untrusted chain and `CaUsedAsEndEntity`")
            L.append("  is a CA certificate served as a leaf. Neither is a SAN")
            L.append("  mismatch, so expect these hosts to keep failing.")
        else:
            L.append("  One caveat on the correct remedy: `caCertificateRefs` alone")
            L.append("  cannot fix a host returning `CaUsedAsEndEntity`, because")
            L.append("  trusting the CA does not make that certificate valid as an")
            L.append("  end-entity. That host needs its certificate fixed.")
        L.append("  Affected hosts, with the number of kept (plugin, route) rows")
        L.append("  dialling each -- fallback upstreams included:")
        for h, n in sorted(rep.private_ca_hosts.items()):
            L.append("  - `%s` -- %d" % (h, n))
    elif rep.private_ca_hosts:
        ...
        L.append("  `policies.tls.caCertificateRefs` naming a ConfigMap with the")
        L.append("  private CA (correct), or `--insecure-upstream-tls All` on this")
        L.append("  generator (matches what Kong effectively does today). Affected")
        L.append("  hosts, with the number of kept (plugin, route) rows dialling")
        L.append("  each -- fallback upstreams included:")
        for h, n in sorted(rep.private_ca_hosts.items()):
            L.append("  - `%s` -- %d" % (h, n))
...
def main():
    ...
    ap.add_argument("--insecure-upstream-tls", choices=("All", "Hostname"),
                    default=None, metavar="MODE",
                    help="emit policies.tls.insecureSkipVerify=MODE for the "
                         "hosts in PRIVATE_CA_HOSTS, whose chain does not "
                         "verify against the system trust store. Kong's "
                         "ai-proxy does not verify at all, so `All` is "
                         "bug-compatible with production today; the correct "
                         "remedy is policies.tls.caCertificateRefs. Scoped to "
                         "the measured hosts -- every other upstream still "
                         "verifies. `Hostname` addresses neither measured "
                         "failure; see the report it writes.")
    ...
    # Kong `fallbacks` become two-knob failover on BOTH surfaces:
    #  - LEGACY (emit_backends): lower-priority `spec.ai.groups`, as before.
    #  - CANONICAL (emit_models): `virtualModel.failover.targets[]` (priority-
    #    ordered, primary=p0, fallbacks increment) -- NOT `weighted`, which would
    #    need weights a Kong fallback chain does not state. The earlier comment
    #    ("virtualModel.weighted.targets[] needs weights ... refuses to invent
    #    them") was wrong: `virtualModel.failover.targets[].priority` takes
    #    order, not weight.
    # Concrete targets carry `policies.health` (knob 1) and a Gateway-wide
    # `AgentgatewayPolicy` with `traffic.retry` (knob 2) is emitted by
    # emit_failover_policies. Both are required: measured 2026-08-27, health
    # alone leaves the 503 with the client (no retry), retry alone redials the
    # same target (no health-gated advance).
    for r in kept:
        fbs = r.cfg.get("fallbacks")
        if isinstance(fbs, list) and fbs:
            rep.loss("Kong `fallbacks` failover now emitted as two-knob failover",
                     "`%s` / `%s` -- %d fallback target(s) emitted as "
                     "`virtualModel.failover` (canonical) + priority `spec.ai.groups` "
                     "(legacy), with `policies.health` on each target and a Gateway-wide "
                     "`traffic.retry` AgentgatewayPolicy. This was previously recorded as "
                     "a hard gap; it is closed at the generator level. Remaining caveat: "
                     "Kong `failover_on` HTTP statuses are mapped to both the health "
                     "`unhealthyCondition` and the retry `codes`, so a status in that "
                     "set both evicts the target and triggers a retry -- verify the "
                     "two-knob interaction under load (probe P4)."
    ...
    backends_yaml = emit_backends(backends, creds, target_ns, rep,
                                  args.insecure_upstream_tls)
    models_yaml = emit_models(models, creds, target_ns,
                              args.gateway_name, args.gateway_namespace, rep,
                              args.insecure_upstream_tls)
    failover_policies_yaml = emit_failover_policies(models, target_ns,
                                                    args.gateway_name, rep)
    ...
    write("failover-policies.yaml", failover_policies_yaml)
