...
async fn make_backend_call(
	...
	mut backend_store_key: Option<&BackendKey>,
) -> Result<Response, ProxyResponse> {
	let resolved_backend;
	let resolved_store_key;
	let backend = if let Backend::LLMRouter(_, router) = backend {
		...
		resolved_store_key = match &resolved.backend.target {
		...
		backend_store_key = resolved_store_key.as_ref();
		resolved_backend.as_ref()
	} else {
		resolved_store_key = None;
		backend
	};
	...
	let (mut backend_call, mut maybe_inference) = match backend {
		Backend::AI(n, ai) => {
			...
			// Also bridge upstream's sessionAffinity: when sticky is not configured
			// but sessionAffinity is, use its affinity key for consistent-hash
			// selection so requests with the same session value route to the
			// same provider.
			let sticky_key = policies.sticky.as_ref().and_then(|_| {
				req.extensions().get::<crate::http::apikey::Claims>()
					.map(|c| c.key.sha256())
			});
			let affinity_key = if sticky_key.is_none() {
				policies
					.session_affinity
					.as_ref()
					.and_then(|p| p.affinity_key(&req))
					.map(|k| k.to_string())
			} else {
				None
			};
		...
		Backend::Dynamic(_, expr) => {
			...
			let backend_call = BackendCall::from_shared(target, policies.clone());
			(backend_call, None)
		},
	...
	let mut resp = resp?;
	// G6: pin-on-success — write key→endpoint pin for sticky/session affinity.
	if resp.status().is_success() {
		if let Some(log) = log.as_ref() {
			if let (Some(key), Some(endpoint)) = (&log.selection_key, &log.selection_endpoint) {
				// Pin when either sticky or sessionAffinity is configured.
				let ttl = policies
					.sticky
					.as_ref()
					.and_then(|s| s.ttl)
					.unwrap_or(std::time::Duration::from_secs(600));
				if policies.sticky.is_some() || policies.session_affinity.is_some() {
