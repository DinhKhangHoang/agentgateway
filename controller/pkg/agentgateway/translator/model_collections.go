...
func translateModelRouteAIPolicy(ctx RouteContext, namespace string, policies *agentgateway.ModelPolicies) (*api.BackendPolicySpec_Ai, error) {
	if policies == nil || (len(policies.Transformations) == 0 && len(policies.Routes) == 0) {
		return nil, nil
	}
	translated, err := translateInlineModelBackendPolicy(ctx, namespace, &agentgateway.BackendFull{
